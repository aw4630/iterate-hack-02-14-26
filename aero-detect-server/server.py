"""
AeroDetect server: YOLOv3 (Darknet) inference via OpenCV DNN for aircraft component detection.
Serves POST /detect with image (JSON body: { "image": "<base64>" }) -> { "items": [{ "label", "bbox": { "x","y","width","height" } }] }.
Expects model config and weights at the configured paths (or set AERO_DETECT_DIR / AERO_DETECT_WEIGHTS).
"""
import os
import base64
import json
...
# Paths: AeroDetect model directory
ROOT = Path(__file__).resolve().parent
AERO_DETECT_DIR = Path(os.environ.get("AERO_DETECT_DIR", ROOT.parent / "AeroDetect"))
CONFIG = AERO_DETECT_DIR / "darknet_configs" / "yolov3_custom_test.cfg"
WEIGHTS = Path(os.environ.get("AERO_DETECT_WEIGHTS", str(AERO_DETECT_DIR / "backup" / "aero-detect_final.weights")))
...
def load_classes():
    ...
    if CLASSES_FILE.exists():
        ...
        # Fallback 25 classes (aircraft components)
        class_names = [
            "engine-cowling", "propeller", "landing-gear-strut", "nose-wheel", "main-wheel",
            "brake-assembly", "exhaust-stack", "oil-filter", "air-filter", "spark-plug",
            "magneto", "carburetor", "fuel-line", "fuel-drain-valve", "wing-strut",
            "aileron", "flap", "elevator", "rudder", "trim-tab",
            "pitot-tube", "navigation-light", "avionics-panel", "battery", "alternator",
        ][:25]
    return class_names
...
def load_net():
    ...
    if not CONFIG.exists():
        ...
        print("Ensure AeroDetect model config exists at the expected path.", file=sys.stderr)
        return False
    if not Path(WEIGHTS).exists():
        print(f"Weights not found: {WEIGHTS}", file=sys.stderr)
        print("Train the AeroDetect model or download weights to the backup directory.", file=sys.stderr)
        return False
    load_classes()
...
def run_detect():
    if not load_net():
        return jsonify({"error": "Model not loaded. Check AeroDetect path and weights."}), 503
...
if __name__ == "__main__":
    ...
    print(f"AeroDetect server starting on port {port}. Model dir: {AERO_DETECT_DIR}")
    if load_net():
        print("Model loaded.")
    else:
